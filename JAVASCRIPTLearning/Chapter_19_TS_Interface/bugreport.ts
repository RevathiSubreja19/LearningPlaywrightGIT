...
console.log("BUG- Report -> " + bug.id + " [" + bug.severity + "] " + bug.title);
    bug.stepsToReproduce.forEach(function (x: string, i: number) {
        console.log("  " + (i + 1) + ". " + x);
    })
}
...
logBug({
    ...
    severity: "High",
    "stepsToReproduce": ["Step1 : open the app.vwo.com", "Step2 :  enter invalid credes", "step3 : verify the error message"]
});
